...
void CheckBuyBreakEvenStop(double Ask)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--) // count all currency pair positions
   {
      ...
      StopLossPips = NormalizeDouble((PositionBuyPrice - PositionStopLoss)/_Point, _Digits);
      
      double BEPips = MathRound(StopLossPips);
      ExtraPips = 10;
...
void CheckSellBreakEvenStop(double Bid)
{
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)  // count all positions
   {
      ...
      StopLossPips = NormalizeDouble((PositionStopLoss - PositionSellPrice)/_Point, _Digits);
      
      double BEPips = MathRound(StopLossPips);
      ExtraPips = 10;
     // double BreakEvenPips = NormalizeDouble((PositionSellPrice - Bid)/_Point, _Digits);
     // Print("Breakevenpips are ", BreakEvenPips);
